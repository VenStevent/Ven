...
// redisSet menerima VALUE MENTAH (object/array) dan dia sendiri yang melakukan
// satu kali JSON.stringify. JANGAN JSON.stringify() lagi di pemanggil — itu
// menyebabkan double-encoding (lihat catatan sama di api/presence-log.js).
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Helper: data lama sempat tersimpan DOUBLE-ENCODED akibat bug
    // double-stringify yang sudah diperbaiki (lihat catatan di redisSet di
    // atas). Supaya data lama yang sudah korup di Redis tidak membuat cron
    // ini crash, parse ulang sampai hasilnya bukan string lagi (maksimal 2x
    // cukup untuk kasus ini, dibatasi biar tidak infinite loop kalau ada
    // string yang memang valid apa adanya).
    ...
    let ids = safeParse(rawIds, []);
    if (!Array.isArray(ids) || ids.length === 0) {
      return res.status(200).json({ ok: true, message: "Tidak ada akun untuk dicek" });
    }

    let prevPresence = safeParse(rawPrev, {});
    let lastSeen = safeParse(rawLastSeen, {});
    // log kamu sempat korup jadi array campuran (entry object asli + pecahan
    // karakter satu-satu dari string yang ke-stringify berkali-kali). Saring
    // supaya cuma entry object yang valid (punya userId) yang dipakai —
    // entry "sampah" (karakter string lepas) otomatis hilang saat disimpan
    // ulang lewat redisSet yang sudah diperbaiki.
    const rawLogParsed = safeParse(rawLog, []);
    const log = Array.isArray(rawLogParsed)
      ? rawLogParsed.filter(entry => entry && typeof entry === "object" && "userId" in entry)
      : [];
    ...
    const saves = [redisSet(PREV_PRESENCE_KEY, prevPresence)];
    if (changed || pruned) {
      saves.push(redisSet(LASTSEEN_KEY, lastSeen));
    }
    if (changed) {
      saves.push(redisSet(LOG_KEY, newLog.slice(0, MAX_LOG)));
    }
