    ...
    try {
      ...
      setAccounts(prev => prev.map(acc => {
        ...
        if (prevP !== undefined && prevP !== p.userPresenceType) {
          ...
          // Update lastOnline di UI sekarang juga (optimis, cuma di state lokal tab ini).
          // Sumber kebenaran lastSeen yang tersimpan di DB tetap dari cron.js;
          // ini cuma supaya tampilan langsung berubah tanpa nunggu cron jalan.
          if (p.userPresenceType === 0) {
            setAccounts(prev => prev.map(a =>
              a.userId === acc.userId ? { ...a, lastOnline: changeTime } : a
            ));
          }
        ...
        // Pakai lastOnline dari Roblox kalau ada, kalau tidak jaga yang lama
        const newLastOnline = p.lastOnline || acc.lastOnline;
